...
# Colunas numéricas para análise principal (baseado nos seus dados)
colunas_numericas_analise = ['price_y', 'quantity', 'price_x']

# Filtrar para usar apenas colunas que realmente existem e são numéricas após conversão
colunas_numericas_analise = [col for col in colunas_numericas_analise if col in df.columns and pd.api.types.is_numeric_dtype(df[col])]

if not colunas_numericas_analise:
    print("\nNENHUMA COLUNA NUMÉRICA VÁLIDA ('price_y', 'quantity', 'price_x') encontrada para análise após as conversões. Verifique os passos anteriores.")
else:
    print(f"\nColunas numéricas selecionadas para análise detalhada: {colunas_numericas_analise}")

    print("\n--- Estatísticas Descritivas Detalhadas ---")
    for col in colunas_numericas_analise:
        print(f"\nAnálises para a coluna: {col}")
        col_data = df[col].dropna() # Remove NaNs para os cálculos individuais

        if col_data.empty:
            print(f"Coluna {col} está vazia ou contém apenas NaNs após dropna(). Pulando cálculos.")
            continue
        ...
        q2 = col_data.quantile(0.50) # Mediana
